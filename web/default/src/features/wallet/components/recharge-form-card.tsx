...
export function RechargeFormCard({
  ...
  // LLMShare: per-topup maximum from /api/status (display unit; 0 = unlimited).
  const { status } = useStatus()
  const rawMaxTopup = (status?.max_topup as number | undefined) ?? 0
  const maxTopup = rawMaxTopup > 0 ? rawMaxTopup : 0
  const aboveMax = maxTopup > 0 && topupAmount > maxTopup
              ...
              <div className='space-y-2.5 sm:space-y-3'>
                ...
                {maxTopup > 0 ? (
                  <div
                    className={cn(
                      'text-xs',
                      aboveMax ? 'text-amber-600 dark:text-amber-400' : 'text-muted-foreground'
                    )}
                  >
                    {aboveMax
                      ? t('Exceeds the single top-up maximum of {{n}}', { n: maxTopup })
                      : t('Single top-up maximum: {{n}}', { n: maxTopup })}
                  </div>
                ) : null}
                ...
                {hasStandardPaymentMethods ? (
                  <div className='grid grid-cols-2 gap-1.5 sm:gap-3 lg:grid-cols-3'>
                    {topupInfo?.pay_methods?.map((method) => {
                      ...
                      const disabled = minTopup > topupAmount || aboveMax
