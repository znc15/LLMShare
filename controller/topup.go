...
// getMaxTopup returns the per-topup maximum in the display unit. 0 = unlimited.
// LLMShare: mirrors getMinTopup's token-mode scaling.
func getMaxTopup() int64 {
	maxTopup := common.MaxTopUp
	if maxTopup <= 0 {
		return 0
	}
	if operation_setting.GetQuotaDisplayType() == operation_setting.QuotaDisplayTypeTokens {
...
func RequestEpay(c *gin.Context) {
	...
	if maxTop := getMaxTopup(); maxTop > 0 && req.Amount > maxTop {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": fmt.Sprintf("单次充值数量不能超过 %d / single topup cannot exceed %d", maxTop, maxTop)})
		return
	}
