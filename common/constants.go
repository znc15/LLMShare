...
// ---- Dynamic Quota (LLMShare) ----
// All amounts are stored in "cents" (1 USD = 100 cents) and converted to/from
// NewAPI's internal quota via QuotaPerUnit at use time. B = hourly pool, F =
// per-user floor, C = per-user cap, lookback = demand window in hours.
var DynamicQuotaEnabled = true
var DynamicQuotaPoolB = 5000      // $50.00 / hour across all users
var DynamicQuotaFloorF = 50       // $0.50 / hour guaranteed per user
var DynamicQuotaCapC = 500        // $5.00 / hour max per user
var DynamicQuotaLookbackHours = 24
var InactivityThresholdDays = 7
var TotalUserCap = 50
var MagicLinkTTLHours = 24
// Channel budget: a single cap (ChannelBudgetCap, USD cents) that applies over a
// selected period (ChannelBudgetCapPeriod: "daily" or "hourly"). The channel is
// rotated away (failover) once its spend in the active window hits the cap.
var ChannelBudgetCap = 10000 // $100.00 per period per channel
var ChannelBudgetCapPeriod = "daily"
// MaxTopUp: maximum amount a user may recharge in a single topup (USD cents).
// 0 = unlimited. Enforced server-side in RequestEpay / RequestAmount.
var MaxTopUp = 0
