...
func InitOptionMap() {
	...
	// LLMShare: default home page to the bundled LittleSheep landing page.
	// Served same-origin at /home.html so it can sync the parent theme via
	// postMessage. Admins can override in System Settings → Content.
	common.OptionMap["HomePageContent"] = "/home.html"
...
// GetAllOptions returns every option row, for export/backup. Admin-only caller.
func GetAllOptions() ([]Option, error) {
	var options []Option
	err := DB.Order("`key` ASC").Find(&options).Error
	return options, err
}
